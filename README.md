...
**Status:** Checkpoint 7 (interface and hardening) is **merged upstream** via [PR #10](https://github.com/jsisiwb/New/pull/10) at `59d62752f31261a0c86921603993f37992194dd2`. **Phase 4 — MVP hardening is ACTIVE and incomplete.** Its credential-free DETERMINISTIC portions are now **merged upstream** via [PR #12](https://github.com/jsisiwb/New/pull/12) at merge commit `99e6bf5ccf962c2283589745ccbef7983cb682d6`, which is the head of the upstream base/default development branch `hoplite/ainos-1ac771f8`; post-merge CI, planning validation and secret scanning all passed. Merged deterministic work: the 120-chapter continuity replay, the deterministic chaos/provider-fallback drills, a disposable-PostgreSQL logical backup/restore drill, the defensive security suite with CI gates, blinded reviewer-packet tooling, and deterministic attempt-level cost accounting. A release-gate review of that branch repaired two defects: **D-6**, where fractional provider cost was truncated so every real call reported as zero spend (now exact integer millicents internally, cents at the boundary), and **D-7**, where `pnpm check` omitted all five hardening suites despite being documented as CI-minus-gitleaks. Baseline evidence at that merge: **67 test files / 985 tests passing / 0 skipped** against PostgreSQL 16, with 40 contrast sets, 800 evaluations, 280/280 agreement and zero false positives or negatives. **No live-provider calls and no credentials** were used: every model call in tests and CI is replayed, and the gateway contains no HTTP client. **Phase 4 remains incomplete** — the live 20-chapter five-night run, the real-provider outage drill, staging/production restore, PITR, live credential rotation and real billing calibration are all still unrun, and **B-4-5 remains incomplete** — its automated portion **B-4-5a is now implemented**: continuation work in the writable fork [`sigma29web/New`](https://github.com/sigma29web/New) expanded the contrast corpus from 40 to **100 accepted sets** (`cs-041`–`cs-100`), authored in validated five-set batches under the deterministic distinctness/coverage gate, giving 2,000 evaluations at 700/700 agreement with zero false positives or negatives. That is an automated corpus result and **not human calibration**: no bilingual reviewer round has run, no reviewer has seen a packet, no judgment exists, and evaluator thresholds remain **`uncalibrated`** (ADR-0029). Other limitations stay open and recorded rather than closed: metrics and rate limiting are per process; fencing prevents a stale commit but cannot abort an already-running provider request; vector retrieval is an interface only; no production deployment has occurred. Nothing here is evidence of live-model prose quality. See `docs/08-delivery/09-progress.md` for the authoritative, itemized state, test inventory and CI evidence per PR.
