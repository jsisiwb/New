...
describe('cli commands', () => {
  ...
  it('compiles the fixture identity block and lists the prompt set', () => {
    ...
    expect(p.ok).toBe(true);
    // 31 English lineage versions + 25 Korean v2.0.0 + v2.1.0 + v2.2.0 + v2.2.1 + v2.2.2 + v2.2.3 + v2.2.4 + v2.2.5 each (ADR-0054).
    expect((p.output as { versions: unknown[] }).versions).toHaveLength(231);
  });
