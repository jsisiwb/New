# ADR-0108: G14a, G14r, G18r and G19r stay as the record of their policies; no override, no regeneration
...
Four chapter-1 runs are stopped at the grant cap, with every gate passing and only reviewer-class findings, or one
...
**None of the four chapters is overridden or regenerated. They stay as the recorded outcome of their policies.**
...
1. **No override.** An override (ADR-0042) is a reviewer's judgement that a finding stands but may be accepted. For
   ...
   problem, and `standard@24` has no second reading to settle it. For G18r and G19r the remaining findings are real
...
2. **No regeneration on the old pins.** A project keeps its policy for life (ADR-0053). Regenerating chapter 1 of G14a
   ...
   them.
3. **Where the lines continue.** The academy line continues on G17a (`standard@28`, chapter 1 accepted) and on a fresh
   `standard@29` academy project; the regression line on a fresh `standard@29` regression project. The four projects are
...
- The four projects remain the measured baseline of `standard@24` and `standard@28` at their grant caps.
