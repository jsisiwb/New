...
| Remaining Checkpoint 7 scope | **not delivered**: the remaining resource families of the API plan (spec/assumptions, concepts, bible, plans, production mutations, canon correction/rollback/dependency promotion, costs/budgets), `apps/web` and all 11 UI screens, correction/regeneration/retcon operator flows, observability/metrics, rate limiting, and the deployment/operator runbooks. None of this is claimed as done |

Honest scope note (ADR-0043): Checkpoint 7 is the largest checkpoint in the roadmap. What exists today is
its platform and API foundation plus job control, SSE, accepted-only export and durable orchestration,
