...
## Phase 4b — Product completion (ADR-0051)

| ID | P | Item | Est |
| --- | --- | --- | --- |
| B-7-1 | P0 | Novel run lifecycle: intake → story suggestions → operator approval → full Story Bible (cast, world, progression system, series blueprint, promises, seed canon) → autopilot chapter production with pause/resume/cancel, over a Postgres-queued runner that needs no Temporal. **Implemented** (migration 0019, `packages/workflows/src/{story-plan,novel,novel-runner}.ts`, API `/v1/projects/:id/novel*`, CLI `novel:*`, web "New novel"). Remaining: a recorded run against a real provider | 5d |
| B-7-2 | P0 | Live provider adapters (OpenAI-compatible, Anthropic) with name-only configuration, classified failures, cancellation, bounded bodies, unknown-usage truthfulness and an optional fallback provider. **Implemented**; the live 20-chapter validation (B-4-1) still needs paid access | 2d |
| B-7-3 | P1 | Live-output normalization: scene drafts re-segmented from prose, evidence spans re-anchored to their quotes, verbatim hook fallback, gateway JSON extraction and `output_mode` repair, chapter planner shown registry ids / knowledge / promises. **Implemented** | 2d |
