...
| --- | --- | --- |
| **OUTPUT-EN-001** | Reader-facing manuscripts (chapter text, chapter titles, in-world text such as status windows and letters) are composed **directly in natural, idiomatic English**. English is the required output language; the Output-Language Profile records the spelling locale (`en-US` default, `en-GB` optional). The architecture keeps the output language an explicit, versioned contract so other languages could be added later, but no such extensibility may weaken or obscure the English requirement in MVP/Beta/Production. | M/P0 |
...
| FR-4.10 | Every manuscript-producing role's output passes a deterministic **output-language check** (language identification ≥ 0.99 English on prose segments excluding approved preserved-script terms) before any further evaluation; failure is blocking and triggers regeneration (OUTPUT-EN-001, NO-TRANSLATION-001). | M/P0 |
